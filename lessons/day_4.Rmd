...
We are working with a subset of the MICS6 data from a few randomly selected countries. How many different countries are in the `country` column? Remember, outside of a tidyverse "pipe" (`%>%`), you can access the values in a particular column from a dataset called `df` with `df$country`.
...
Create a new dataset by filtering the data to observations from one country of your choice from Question 2. Then, calculate the average `numeracy` only within that one country.
...
Using `group_by()` and `summarise()` together, calculate the average `numeracy` in each country from the MISC6 dataset. For a reminder on how these functions work, feel free to consult Notebook #4 from the [Summer Assignment](https://colab.research.google.com/drive/17AHZZiT2VSMPmJNIM1TaCb08R9iqkAgq?authuser=1#scrollTo=4OIE4FE4ZYwb), use Google, or ask a group member / teaching staff.
