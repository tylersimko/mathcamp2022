...
# Write code here then delete this line
```

#### Question 2

We are working with a subset of the MICS data from a few randomly selected countries. How many unique countries are in the `country` column? Remember, when not using a tidyverse "pipe" (`%>%`), you can access the values in a particular column from a dataset called `df` with `df$country`.
...
Using functions with the `%>%`, calculate the average `numeracy` in each country from the MICS dataset. For a reminder on how these functions work, feel free to consult Notebook #4 from the [Summer Assignment](https://colab.research.google.com/drive/17AHZZiT2VSMPmJNIM1TaCb08R9iqkAgq?authuser=1#scrollTo=4OIE4FE4ZYwb), use Google, or ask a group member / teaching staff.
