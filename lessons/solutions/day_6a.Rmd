...
The line below will read a subset of the `MICS` data used last week with three countries. First, find which countries are in the data. Then, use the examples above of making datasets (e.g. `pop`, `cities`, `students`, and `grades`) to create a new dataset with `tibble()` that contains country names and capital cities for each of the countries in the `MICS` data (feel free to Google for capital city names!). Finally, merge the dataset below with the new tibble you create. Your final answer should have all of the same columns as the `MICS` data with an additional column for the capital city of that country.

```{r}
mics <- read_csv("https://www.dropbox.com/s/bhenqxcdcklwvgg/mics6_sub.csv?dl=1")

cities <- tibble(country = c("Ghana", "Mongolia", "Nepal"),
       capital = c("Accra", "Ulaanbaatar", "Kathmandu"))

full_join(mics, cities, by = "country")
```
...
df %>% 
  group_by(country) %>% 
  summarise(unique(continent))
```

#### Question 4

Make a histogram that shows the number of children in each value of `highest_grade` on a separate facet for the two continents we merged in. Fill the histogram with a HEX color of your choice ([this](https://htmlcolorcodes.com/) is a good site) and set a `binwidth` of 0.5.

```{r}
df %>% 
  ggplot(aes(x = highest_grade)) + 
    geom_histogram(fill = "#854095", binwidth = 0.5) + 
...
**Hint**: try finding the section of the documentation that is specifically about the argument we use to specify join IDs.
